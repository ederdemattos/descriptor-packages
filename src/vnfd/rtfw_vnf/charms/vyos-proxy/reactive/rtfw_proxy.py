...
def test_connection():
    ...
    remove_flag('rtfw-proxy.ready')
    try:
        who, _ = run('whoami')
    except MgmtNotConfigured as e:
        remove_flag('rtfw-proxy.configured')
        status_set('blocked', str(e))
    except subprocess.CalledProcessError as e:
        remove_flag('rtfw-proxy.configured')
        status_set('blocked', e.output)
    else:
        set_flag('rtfw-proxy.configured')


@when('rtfw-proxy.configured')
@when_not('rtfw-proxy.ready')
def rtfw_proxy_ready():
    status_set('active', 'ready')
    set_flag('rtfw-proxy.ready')


@when('actions.ping')
@when_not('rtfw-proxy.configured')
def pingme():
